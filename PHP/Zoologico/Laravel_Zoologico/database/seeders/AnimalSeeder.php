...
Son diurnos y viven preferentemente en el suelo. Forman grupos numerosos que se hallan socialmente bien organizados, liderados por un fuerte macho. Poseen por tanto sus propias jerarquías, hábitos y costumbres.'
    ...
    public function run(): void
    {
        foreach ($this->animales as $animal) {
            $a = new Animal(); // Instancia del modelo animal, representa a la tabla animals en la base de datos
            $a->especie = $animal['especie'];
            $a->slug = Str::slug($animal['especie']); //En este caso se usa Str para convertir la especie del animal (un string) en un slug amigable para URL. Ej: León Africano->leon-africano
            $a->peso = $animal['peso'];
