...
<div class="flex flex-col md:flex-row md:space-x-8 space-y-8 md:space-y-0">
    ...
    <div class="w-full md:w-1/2 bg-gray-100 p-6 rounded-lg shadow-lg">
        ...
        <div class="mt-6">
            <img src="{{ asset('assets/imagenes/'.$animal->slug.'.jpg') }}" alt="{{ $animal->especie }}" class="h-48 w-48 object-cover rounded-lg shadow-lg mx-auto">
        </div>
    </div>

    {{-- Formulario para editar el animal --}}
    <form action="{{ route('animales.update', $animal) }}" method="POST" enctype="multipart/form-data" class="w-full md:w-1/2 bg-white p-6 rounded-lg shadow-lg">
        @csrf
        @method('put')
        ...
        <div class="mb-4">
            ...
            <textarea name="descripcion" id="descripcion" rows="4" class="w-full p-3 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500">{{ $animal->descripcion }}</textarea>
        </div>
